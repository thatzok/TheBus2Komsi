...
pub fn compare_vehicle_states(
    ...
    if (old.ignition != new.ignition) || force {
        ...
        let mut b = build_komsi_command_u8(KomsiCommandKind::Ignition, new.ignition, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.engine != new.engine) || force {
        ...
        let mut b = build_komsi_command_u8(KomsiCommandKind::Engine, new.engine, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.doors != new.doors) || force {
        ...
        let mut b = build_komsi_command_u8(KomsiCommandKind::PassengerDoorsOpen, new.doors, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.fixing_brake != new.fixing_brake) || force {
        if verbose {
            ...
            let mut b =
                build_komsi_command_u8(KomsiCommandKind::FixingBrake, new.fixing_brake, &opts);
            buffer.append(&mut b);
        }
    }
    ...
    if (old.indicator != new.indicator) || force {
        ...
        let mut b = build_komsi_command_u8(KomsiCommandKind::Indicator, new.indicator, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.lights_warning != new.lights_warning) || force {
        ...
        let mut b =
            build_komsi_command_u8(KomsiCommandKind::LightsWarning, new.lights_warning, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.lights_main != new.lights_main) | force {
        ...
        let mut b = build_komsi_command_u8(KomsiCommandKind::LightsMain, new.lights_main, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.fuel != new.fuel) || force {
        ...
        let mut b = build_komsi_command(KomsiCommandKind::Fuel, new.fuel, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.speed != new.speed) || force {
        ...
        let mut b = build_komsi_command(KomsiCommandKind::Speed, new.speed, &opts);
        buffer.append(&mut b);
    }
    ...
    if (old.maxspeed != new.maxspeed) || force {
        ...
        let mut b = build_komsi_command(KomsiCommandKind::MaxSpeed, new.maxspeed, &opts);
        buffer.append(&mut b);
    }
